...
3. When a Douyin URL is provided, the live room must be opened in Chrome because the comment crawler depends on Chrome CDP. Start capture with the Douyin live comment crawler unless the user disables it. Store comments as `<直播间名称>-<yyyyMMdd-HHmmss>-comments.jsonl` in the same room archive folder.
...
3. Choose the source workflow.
   ...
   - For URL-triggered recording: run `scripts/start-live-monitor.ps1 -Url <url> -Mode video` for screen plus system audio, or `-Mode audio` for system audio only. When `-RoomName` or `-GameProductName` is omitted, the script first runs `scripts/resolve-douyin-live-metadata.py` for about 10 seconds to infer the live room name, game product name, and visible viewer count; explicit user-provided values always win. Douyin live URLs must be opened in Chrome rather than the default browser because the comment crawler depends on Chrome CDP. This starts the bundled `scripts/douyin-comment-crawler.py` and writes `<直播间名称>-<yyyyMMdd-HHmmss>-comments.jsonl` unless `-DisableComments` is passed; if comments are disabled or the crawler fails to start, the script still opens the live room in Chrome for recording. Wait for the user's "暂停" command, then run `scripts/stop-live-monitor.ps1`.
