...
- 输入直播间链接后录屏：画面 + 系统声音。
- 输入直播间链接后录音：仅系统声音。
- 输入抖音直播链接后统一用 Google Chrome 打开直播间，保证评论爬虫可连接 Chrome CDP。
- 停止录制后自动抽取音频并转写文本。
- 同步抓取抖音直播评论 JSONL。
...
| Google Chrome | 必需 | 打开抖音直播间，供评论爬虫通过 Chrome CDP 连接 |
...
发送直播链接后，AI/脚本必须用 Google Chrome 打开直播间；不要改用 Edge、系统默认浏览器或手动另外打开多个页面，否则评论爬虫无法稳定连接直播页。
